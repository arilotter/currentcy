...
function updateRates(rates) {
  for (const currIn in rates) {
    for (const currOut in rates[currIn]) {
      if (Date.now() - Number(rates[currIn][currOut][1]) >= 1800000) {
        // 30 minutes
        if (Date.now() - Number(rates[currOut][currIn][1]) < 1800000) {
          rates[currIn][currOut][0] = (
            1 / Number(rates[currOut][currIn][0])
          ).toString(); // Value
          rates[currIn][currOut][1] = Number(
            rates[currOut][currIn][1]
          ).toString(); // Timestamp
    ...
    fetch(jsonSource)
      .then(function(res) {
        ...
        res.json().then(function(data) {
          ...
          console.log(
            `rates for ${currIn} to ${currOut} set from ${jsonSource}.`
          );
        });
      })
      .catch(function(err) {
