...
function getPrice(nodeVal) {
    ...
    if (sep === ".") {
        ...
        price = parseFloat(nodeVal.replace(".", "").replace(",", ".").replace("-", "0"));
    } else if (!sep) {
...
function convertRates(rates) {
    ...
    for (let i = 0; i < getText.length; i++) {
        for (let j = 0; j < currencies.length; j++) {
            ...
            if (match = new RegExp("(?=.*\\d)^" + currencies[j][0] + "(\\s)?(\\d{0,3}((\\.|,)\\d{3})*)?((\\.|,)(\\d{1,2}|\\-{2}))?$").exec(getText[i].nodeValue)) {
                result = getPrice(match[0].replace(new RegExp(currencies[j][0]), "").replace(" ", ""));
                // e.g. € 19,99, $14.99
            } else if (match = new RegExp("(?=.*\\d)^" + currencies[j][1] + "(\\s)?(\\d{0,3}((\\.|,)\\d{3})*)?((\\.|,)(\\d{1,2}|\\-{2}))?$").exec(getText[i].nodeValue)) {
                result = getPrice(match[0].replace(new RegExp(currencies[j][1]), "").replace(" ", ""));
                // e.g. 19,99€, 14.99 $
            } else if (match = new RegExp("(?=.*\\d)^(\\d{0,3}((\\.|,)\\d{3})*)?((\\.|,)(\\d{1,2}|\\-{2}))?(\\s)?" + currencies[j][1] + "$").exec(getText[i].nodeValue)) {
                result = getPrice(match[0].replace(new RegExp(currencies[j][1]), "").replace(" ", ""));
            }
